...
public class SlideMenu extends LinearLayout {
	...
	private static class SlideMenuAdapter extends ArrayAdapter<SlideMenuItem> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (rowView == null) {
				...
				viewHolder.tdesc=(TextView) rowView.findViewById(R.id.menu_desc);
			...
			holder.tdesc.setText(items[position].desc);
	...
	public void init(Activity act, int menuResource, SlideMenuInterface.OnSlideMenuItemClickListener cb, int slideDuration) {
		...
		// set size
		menuSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 255, act.getResources().getDisplayMetrics());
			...
			while(eventType != XmlPullParser.END_DOCUMENT){
				...
				if(eventType == XmlPullParser.START_TAG){
					...
					if(elemName.equals("item")){
						...
						String desc=	xpp.getAttributeValue("http://schemas.android.com/apk/res/android", "titleCondensed");
						...
						item.desc = resourceIdToString(desc);
						menuItemList.add(item);
					}
